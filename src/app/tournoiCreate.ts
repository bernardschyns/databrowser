...
export function tournoiCreate(ds) {
    ...
    var joueursPrepareSync = (joueursProm) => {
        joueurs = joueursProm.entities

        joueursNb = lengthJoueur = joueurs.length
        joueursEnPlace = []
    ...
    var monTournoiProm = tournoiPrepareAsync()
    monTournoiProm.then(res => {
        ...
        var doCategorieAsync = () => {
            ...
            counterCategorie++
            var joueursProm = joueursPrepareAsync()
            var terrainsProm = terrainsPrepareAsync()
            Promise.all([joueursProm, terrainsProm]).then(
                ([joueursProm, terrainsProm]) => {
                    joueursPrepareSync(joueursProm)
                    terrainsPrepareSync(terrainsProm)
                    matchPrepareSync()
                    ...
                    // todo gérer les catégories sans hardcoding
                    let queryString = "name=='Messieurs2'";
                    ds.Categorie.query({ filter: queryString }).then(collection => {
                        
                        let myCategorie = collection.entities[0]
    ...
    function joueursAdapt() {
        joueurs.map(joueur => {
            ...
            ds.Joueur.query({ filter: queryString, pageSize: 1, select: "ID,nom,horaireCollection:5" }).then(collection => {

                let elem = collection.entities[0]
                let horairesToDelete = elem.horaireCollection.entities;
                horairesToDelete.map(horaire => horaire.delete());
                let matchs = joueur.horaireCollection.entities.filter(horaire => horaire.typeEvent == "match");
                matchs.map(match => {
                    let matchsEvents = match.event.events;
                    let h1 = ds.Horaire.create()
                    h1.joueur = elem;
                    h1.typeEvent = "match";
                ...
                let horaires = joueur.horaireCollection.entities.filter(horaire => horaire.typeEvent == "horaire");
                horaires.map(horaire => {
                    let horairesEvents = horaire.event.events;
                    let h2 = ds.Horaire.create()
                    h2.joueur = elem;
    ...
    function terrainAdapt() {
        terrains.map(terrain => {
            ...
            ds.Terrain.query({ filter: queryString, pageSize: 1, select: "ID,nom,horaireCollection:5" }).then(collection => {
                let elem = collection.entities[0]
                let horairesToDelete = elem.horaireCollection.entities;
                horairesToDelete.map(horaire => horaire.delete());
                let matchs = terrain.horaireCollection.entities.filter(horaire => horaire.typeEvent == "match");
                matchs.map(match => {
                    let matchsEvents = match.event.events;
                    let h1 = ds.Horaire.create();
                    h1.terrain = elem;
                ...
                let horaires = terrain.horaireCollection.entities.filter(horaire => horaire.typeEvent == "horaire");
                horaires.map(horaire => {
                    let horairesEvents = horaire.event.events;
                    let h2 = ds.Horaire.create();
                    h2.terrain = elem;
    ...
    function adaptDispo(elem, ...args) {
        ...
        let horaires = elem.horaireCollection.entities;
    ...
    function classNameHoraire(el, index, className) {
        let elHoraires = el.horaireCollection.entities;
...
//todo terminer l'élimination de date-fns comme montré en ligne 447...//let heureCible en commentaire
    function horaireMatchFind(counter) {
        let reste = counter % 2
        let increment
        increment = (reste == 0) ? Math.floor(counter / 2) : Math.floor(counter / 2)
        let heureRef = moment(dayArray[counterDay])
        ...
        let heureCible =moment(heureRef).hour(heureRef.hour()+translationHeureCible)
        ...
        let secondHeureCible =moment(heureCible).hour(heureCible.hour()+translationHeureEnd)
    ...
    var terrainsPrepareSync = terrainsProm => {
        terrains = terrainsProm.entities
        counterTerrain = -1
