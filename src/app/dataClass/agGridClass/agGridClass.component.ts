        ...
        switch (niveau) {
            ...
            case 4: {
                switch (entitySource['changed']) {
                    case 'add': {
                        debugger
                            entitySource['descriptif']="bonsoir"
                            entitySource = await entitySource.save()
                            entitySource = await this.wakanda.catalog['WakandaLogin'].create(entitySource._dataClass.name, entitySource)
                            .then(res=>{
                                debugger
                                this.updated = true
                                return res
